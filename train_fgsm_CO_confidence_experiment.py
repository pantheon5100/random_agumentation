...
def main():
    ...
    if args.out_align_method != "none":
        ...
        args.out_dir = args.out_dir + f"-out_align_method_{args.out_align_method}-out_align_noise_{args.out_align_noise}-"
    ...
    for epoch in range(args.epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            if not args.supervised:
                ...
                output_clean = model(X + delta[:X.size(0)])
                loss = criterion(output_clean, y)
            ...
            # if args.out_align_method != "none":
            #     loss_out_align = output_align(X, y, output_adv, epsilon, alpha, model, lower_limit, upper_limit, align_method=args.out_align_method, opt=opt, out_align_noise=args.out_align_noise)
            #     if args.out_align_method == "NAE2GT":
            ...
            # Confidence metric
            bs = y.size(0)
            confidence_metric["bs"] += bs
            
            # clean soft
            prob_clean = F.softmax(output_clean, dim=-1)
            avg_prob_clean = prob_clean.mean()
            correct_prob_clean = (F.one_hot(y) * prob_clean).sum(-1).mean()
            confidence_metric["avg_prob_clean"] += avg_prob_clean*bs
            confidence_metric["correct_prob_clean"] += correct_prob_clean*bs


            # adv
            if not args.supervised:
                prob_adv = F.softmax(output_adv, dim=-1)
                avg_prob_adv = prob_adv.mean()
                correct_prob_adv = (F.one_hot(y) * prob_adv).sum(-1).mean()
                confidence_metric["avg_prob_adv"] += avg_prob_adv*bs
                confidence_metric["correct_prob_adv"] += correct_prob_adv*bs
        ...
        writer.add_scalar("avg_prob_clean", confidence_metric["avg_prob_clean"]/confidence_metric["bs"] , epoch)
        writer.add_scalar("correct_prob_clean", confidence_metric["correct_prob_clean"]/confidence_metric["bs"] , epoch)

        if not args.supervised:
            writer.add_scalar("avg_prob_adv", confidence_metric["avg_prob_adv"]/confidence_metric["bs"] , epoch)
            writer.add_scalar("correct_prob_adv", confidence_metric["correct_prob_adv"]/confidence_metric["bs"] , epoch)
