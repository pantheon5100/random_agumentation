...
def get_args():
    ...
    parser.add_argument('--alpha', default=20, type=float, help='Step size')
    parser.add_argument('--delta-init', default='zero', choices=['zero', 'random'],
        help='Perturbation initialization method')
    parser.add_argument('--out-dir', default='generating_examples', type=str, help='Output directory')
...
def main():
    ...
    # FGSM CO model, eps 16 alpha 20, delta init zero
    # testing_file = "log_files/train_fgsm_CO_confidence_experiment-FGSM/train_fgsm_CO_confidence_experiment-FGSM-image_normalize--epochs_30-lr_schedule_cyclic-lr_max_0.3-epsilon_16-attack_steps_7-alpha_20.0-delta_init_zero-zero_one_clamp_1-seed_0/20220119092246/model.pth"
    
    # PGD 2 eps 16 alpha 8 step 2
    testing_file = "log_files/pgd2-baseline-eps16/pgd2-baseline-eps16-image_normalize--epochs_30-lr_schedule_cyclic-lr_max_0.3-epsilon_16-attack_steps_2-alpha_8.0-delta_init_zero-zero_one_clamp_1-seed_3/20220114133508/model.pth"
    ...
    train_loader, test_loader, _ = get_loaders(args.data_dir, args.batch_size, args.image_normalize, cifar10_mean, cifar10_std)
    
    generating_examples = True
    ...
    if generating_examples:
        ...
        # generate trainable sampels based on a CO model
        alpha = (16 / 255.) / std
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            torch.save(delta.cpu(), f"generating_data/generating_CO_examples_data_batch_id_{i}.pth")
            torch.save(y.cpu(), f"generating_data/generating_CO_examples_label_batch_id_{i}.pth")


    time_stamp = datetime.now().strftime("%Y%m%d%H%M%S")

    args.out_dir = os.path.join("generating_examples_logger", args.out_dir, time_stamp)
    ...
    model_train = PreActResNet18().cuda() ### to make sure that the robustness evaluation is done on single precision instead of half-precision

    train_data = [ torch.load(f"generating_data/generating_CO_examples_data_batch_id_{i}.pth") for i in range(len(train_loader))]
    train_label = [ torch.load(f"generating_data/generating_CO_examples_label_batch_id_{i}.pth") for i in range(len(train_loader))]
    criterion = nn.CrossEntropyLoss()

    opt = torch.optim.SGD(model_train.parameters(), lr=args.lr_max, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (data, label) in enumerate(zip(train_data, train_label)):
            ...
            bs = label.size(0)
            train_correct += bs * (output.max(1)[1] == label).sum().item()

            n += bs

        logger.info(f'Epoch: {epoch}. train_acc: {train_correct*100/n:.3f}')
    
        test_acc =  evaluate_standard(test_loader, model_train)

        logger.info(f'Epoch: {epoch}. test_acc: {test_acc[-1]:.3f}')
