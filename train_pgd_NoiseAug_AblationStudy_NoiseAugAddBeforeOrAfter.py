...
def get_args():
    ...
    NOISE_AUG_POSITION = ["before", "after"]
    parser.add_argument('--noise_aug_position', default="before", type=str, choices=NOISE_AUG_POSITION, help='Change noise augmenttion  size.')
...
def main():
    ...
    if args.noise_aug:
        args.out_dir = args.out_dir + f"-NoiseAug-_type_{args.noise_aug_type}-noise_aug_size_{args.noise_aug_size}-noise_aug_position_{args.noise_aug_position}"
    ...
    for epoch in range(args.epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            if args.noise_aug and (args.noise_aug_position == "before"):
            ...
            if args.noise_aug and (args.noise_aug_position == "after"):
                noise = torch.zeros_like(X).cuda()
                if args.noise_aug_type == "uniform":
                    noise.uniform_(-1, 1) 
                elif args.noise_aug_type == "normal":
